...
Inside `$VAULT`, the skill organizes notes by category and maintains one
index file:

```
$VAULT/
├── _index.md       # auto-maintained: tag / entity vocabulary + recent-notes list
├── work/
├── learning/
├── reading/
├── personal/
...
`_index.md` is the controlled-vocabulary registry. Notes themselves are the
canonical data; `_index.md` is a derived view, mechanically regenerated by
`scripts/rebuild-index.py`. `sync.sh` runs the rebuild before every push, so
the index always reflects the truth (the notes) at commit time, and any
note that fails validation (missing frontmatter fields, category-folder
mismatch, malformed filename) blocks the push. The skill itself only does
a best-effort cache update during each session so back-to-back triggers see
fresh vocabulary — the script is the gate.

`_index.md` lives in the vault (private), so any work-specific or otherwise
sensitive terms collected there never appear in the public skill repo.

Category subdirectories are created on demand the first time a note of that
category is written — no setup required.
